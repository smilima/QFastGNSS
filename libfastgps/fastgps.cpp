...
int run_fastgps()
{
    ...
    system_vars.ms_update_flag.store(OFF);
    ...
    // {
    while (!feof(data_file) && (system_vars.process_time < system_vars.run_time) && !get_stop_flag().load())
    {
        // Read Data from file
        ...
        if(system_vars.Rx_State == ACQ)
        {
            ...
            else
            {
                if (acq_buf_write_pos >= system_vars.acq_buf_len)
                {
                    // Create a list of PRNs to search
                    QList<int> prns_to_search;
                    for (j = 0; j < system_vars.num_search_svs; j++) {
                        prns_to_search.append(system_vars.prn_search_list[j]);
                    }

                    // Get the FFT length
                    unsigned acq_fft_len = kiss_fft_next_fast_size(system_vars.acq_buf_len);

                    // Define the parallel map function
                    auto acquire_satellite = [&](int prn) -> AcqResult {

                        // --- SAFER BUFFER HANDLING ---
                        // Create buffers just for this task
                        AcqThreadBuffers buffers = create_acq_thread_buffers(acq_fft_len);

                        // Call the new refactored acquisition task
                        AcqResult result = acquire_satellite_task(prn, buffers);

                        // Free the buffers now that the task is done
                        free_acq_thread_buffers(buffers);

                        return result;
                        // --- END SAFER HANDLING ---
                    };

                    // Run the parallel acquisition
                    QList<AcqResult> results = QtConcurrent::blockingMapped(prns_to_search, acquire_satellite);

                    // --- Process Results Serially ---
                    // This part is safe; only one thread is running here.
                    for (const AcqResult& res : results) {
                        if (res.found) {
                            if (system_vars.num_channels >= MAX_CHANNELS) {
                                break; // We have filled all available channels
                            }

                            unsigned ch_idx = system_vars.search_channel;

                            fastgps_printf("Found PRN %d, allocated to channel %d\n", res.prn, ch_idx);

                            // Populate the global channel 'c[ch_idx]' from the result
                            c[ch_idx].prn_num = res.prn;
                            c[ch_idx].acq.ratio = res.ratio;
                            c[ch_idx].doppler = res.doppler;
                            c[ch_idx].true_car_phase_inc = res.true_car_phase_inc;
                            c[ch_idx].true_code_inc = res.true_code_inc;
                            c[ch_idx].track.carrier_freq = res.track_carrier_freq;
                            c[ch_idx].track.code_freq = res.track_code_freq;
                            c[ch_idx].code_prompt = (CHIPS_PER_CODE - res.code_phase + 1);

                            // ... (copy all other fields from acq_buffer_full2's old init block) ...
                            c[ch_idx].num_ms = 0;
                            c[ch_idx].tracking_lock = 0;
                            c[ch_idx].state = CH_STATE_POSTACQ_SPIN;

                            // Update global state (serially)
                            system_vars.sats_found |= ((uint64_t)1 << res.prn);
                            system_vars.prn_to_channel_map[res.prn] = ch_idx;
                            system_vars.search_channel++;
                            init_correlator_channel(system_vars.search_channel); // Prepare next
                            system_vars.num_channels++;
                        } else {
                            //fastgps_printf("Did not find PRN %d\n", res.prn);
                        }
                    }
                }  // end if (acq_buf_write_pos >= system_vars.acq_buf_len)
                }  // end else (file_acquisition == OFF)
        } // end of if(system_vars.Rx_State == ACQ)
        ...
        if(system_vars.Rx_State >= TRACKING)
        {
            ...
            // 1. Create a list of active channel indices
            ...
            // 2. Define the parallel processing function (as a lambda)
            auto process_channel = [&](int idx) {
                struct channel *ch = &c[idx];

                // Call correlator and tracking functions
                software_correlator(ch, data_buf + ch->acq.acq_finish_time,
                                    bytes_read - ch->acq.acq_finish_time);

                // This check is now a data race, must be done serially after
                // if(ch->nav.nav_data_state >= HAVE_TOW)
                //   system_vars.num_valid_tow++; // DANGER
                // if(ch->nav.nav_data_state == HAVE_EPH)
                //   system_vars.num_valid_eph++; // DANGER

                // only on the first correlation after acquistion do we adjust for
                // acq_finish_time
                ch->acq.acq_finish_time = 0;
            };

            // 3. Run the map function, this blocks until all channels are processed
            QtConcurrent::blockingMap(active_channels, process_channel);

            // 4. Now that processing is done, serially (and safely) check nav state
            for (ch_idx = 0; ch_idx < system_vars.num_channels; ch_idx++)
            {
            ...
            // --- END OF PARALLEL TRACKING LOOP ---

            if ((system_vars.tracking_log_flag == NORMAL_LOGGING &&
                 system_vars.ms_update_flag.load() >= 20) || // Use .load()
                (system_vars.tracking_log_flag == DEBUG_LOGGING &&
                 system_vars.ms_update_flag.load() >= 1)) // Use .load()
            {
                update_tracking_log();
                system_vars.ms_update_flag.store(OFF); // Use .store()
            }
