...
// --- NEW GLOBALS FOR THREAD-SAFE TIME INIT ---
static std::once_flag g_recv_time_init_flag;
static std::mutex g_eph_mutex; // To protect non-atomic global vars
// ---------------------------------------------
...
void process_subframe(struct channel *x)
{
  ...
  // --- THREAD-SAFE RECEIVER TIME INITIALIZATION ---
  // Use call_once to ensure this block is only executed
  // by the first thread that arrives here.
  std::call_once(g_recv_time_init_flag, [&]() {
      if (!system_vars.recv_time_valid)
      {
          // set the receiver clock
          double time_since_tow = (x->clock - x->nav.subframe_start_clock) /
